      ...
      return [
        tag,
        {
          ...
          async load() {
            ...
            if (
              typeof viewConfig.url !== "string" ||
              viewConfig.url.length === 0
            )
              throw new Error(`gams config ui.views.${tag}.url is required`)
            await require(viewConfig.url)
            if (!customElements.get(tag))
              throw new Error(
                `view '${tag}' did not register custom element '${tag}'`,
              )
          },
...
async function applyThemeStylesheet(config) {
  ...
  const readResult = unwrapResult(
    await runtime.invoke("fs/fs::read-file", themePath),
    "theme read",
  )
  ...
  document.querySelectorAll("view-area, view-popup").forEach((el) => {
    const shadowLink = el.shadowRoot?.querySelector(
