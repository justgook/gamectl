...
native_map_action :: proc(key: sapp.Keycode) -> (u32, bool) {
	#partial switch key {
	case .A, .LEFT:
		return ACTION_LEFT, true
	case .D, .RIGHT:
		return ACTION_RIGHT, true
	case .W, .UP:
		return ACTION_UP, true
	case .S, .DOWN:
		return ACTION_DOWN, true
	case .J:
		return ACTION_1, true
	case .K:
		return ACTION_2, true
	case:
		return 0, false
	}
}
...
native_init :: proc "c" () {
	...
	core_handle_framebuffer_resize(i32(sapp.width()), i32(sapp.height()))
}
...
native_event :: proc "c" (e: ^sapp.Event) {
	...
	case .RESIZED:
		...
		core_handle_framebuffer_resize(i32(e.framebuffer_width), i32(e.framebuffer_height))
	case .KEY_DOWN:
		if action, ok := native_map_action(e.key_code); ok {
			core_handle_action_down(action)
		}
	case .KEY_UP:
		if action, ok := native_map_action(e.key_code); ok {
			core_handle_action_up(action)
		}
	case:
	}
}
