...
import { parseArgs, rootPrlModels } from './root-one-fixtures.mjs'
...
const options = parseArgs(process.argv.slice(2), { models: rootPrlModels, runs: 1, steps: 10 })
if (!skipOrRequireOriginalRepo(options, mjRoot)) {
  ...
  console.log(`root-prl parity: models=${options.models.join(',')} runs=${options.runs} steps=${options.steps}`)
