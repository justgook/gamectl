...
import { parseArgs, rootMarkovModels } from './root-one-fixtures.mjs'
...
const options = parseArgs(process.argv.slice(2), { models: rootMarkovModels, runs: 1, steps: 10 })
if (!skipOrRequireOriginalRepo(options, mjRoot)) {
  ...
  console.log(`root-markov parity: models=${options.models.join(',')} runs=${options.runs} steps=${options.steps}`)
