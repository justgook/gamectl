...
import { parseArgs, rootSequenceModels } from './root-one-fixtures.mjs'
...
const options = parseArgs(process.argv.slice(2), { models: rootSequenceModels, runs: 1, steps: 10 })
if (!skipOrRequireOriginalRepo(options, mjRoot)) {
  ...
  console.log(`root-sequence parity: models=${options.models.join(',')} runs=${options.runs} steps=${options.steps}`)
