...
`compiler/xml-to-mjir.mjs` is the reusable XML→MJIR entry point. It currently supports root `<one>`, `<all>`, `<prl>`, root `<markov>`, and root `<sequence>` models with nested `<markov>/<sequence>` containers, inline `in`/`out` patterns or child `<rule>` patterns, `<union>` declarations, resource-free `<field>` declarations on `<one>/<all>` nodes, non-search `<observe>` declarations, resource-free `<path>` nodes, `values`, `origin`, `symmetry`, child `steps`, `temperature`, and rule probability `p` attributes. It emits MJIR v1 pattern rules (`op = 2`), union declarations (`op = 101`), field/temperature declarations (`op = 103/104`), observation declarations (`op = 105`), path payloads (`op = 106`), plus node/container markers (`op = 100`) when needed, and prepares initial indexed grids outside the component.
