...
type TileMatcher struct {
	...
	UsedTiles    map[uint32]bool // For "Other" matching
}
...
// Returns true if tiles match according to special tile rules
...
func (m *TileMatcher) matchesInternal(patternTile, targetTile uint32) bool {
	// 1. Ignore - always matches (skip this cell)
	...
	// (handled via IsNegated flag on layer level)
	if m.SpecialTiles.Negate > 0 && patternTile == m.SpecialTiles.Negate {
		// If this appears in pattern, invert the entire match
		// This is a special case that should be handled differently
		// For now, treat as "never matches" to indicate error
		return false
	}
