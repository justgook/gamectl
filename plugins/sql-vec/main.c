...
// SQL Vec plugin implemented in C with SQLite3 and sqlite-vec
// Demonstrates:
...
static uint32_t sql_register_vec(void) {
  char *vec_err_msg = NULL;
  int rc = sqlite3_vec_init(db, &vec_err_msg, NULL);
  if (rc != SQLITE_OK) {
    if (vec_err_msg) {
      uint32_t err_len = pdk_strlen(vec_err_msg);
      pdk_output((const uint8_t *)vec_err_msg, err_len);
      sqlite3_free(vec_err_msg);
    } else {
      const char error_msg[] = "Failed to initialize sqlite-vec";
...
static uint32_t sql_init_database(void) {
  ...
  // Register sqlite-vec as a statically linked SQLite extension.
  if (sql_register_vec() != 0) {
    return 1;
  }
...
__attribute__((export_name("info"))) uint32_t info(void) {
  const char info_msg[] =
      "SQL Vec plugin v2.1 - SQLite3 with sqlite-vec and mem3 allocator - provides "
