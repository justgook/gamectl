...
- `../CONTEXT.md` — domain glossary only; no implementation decisions or specs.
...
3. Extract domain terms into `CONTEXT.md` as soon as they are resolved.
...
7. Leave the legacy file in place, but mark the corresponding PRD/ADR links in `docs/legacy/INDEX.md` when conversion is complete.
...
- resolved domain terms are in `CONTEXT.md`;
- unresolved questions remain only in PRD `Open questions`, not hidden in legacy prose;
- `docs/legacy/INDEX.md` links the legacy area to the new PRD/ADR/current docs;
