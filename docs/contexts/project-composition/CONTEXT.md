...
- Each GetVar declaration adds an individual named output to its owning For Each Node. After execution, these outputs expose the final committed Iteration State values. A state name must not collide with a Graph Output name in the same For Each.
- A zero-length For Each still initializes Iteration State and executes GetVar initialization dependency chains once. Its final state outputs contain the initial values while collected Graph Outputs remain empty arrays.
