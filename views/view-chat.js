...
function parseToolCommand(input) {
  ...
  const argsText = firstWhitespace === -1 ? '' : body.slice(firstWhitespace + 1).trim()
...
function buildRuntimeArgs(argsTemplate, argv) {
  ...
  return argsTemplate.map((entry) => {
    ...
    if (positional) {
      ...
      if (index < 0) throw new Error(`Invalid positional argument placeholder '${entry}'`)
      if (index >= argv.length) throw new Error(`Missing command argument ${entry}`)
      return argv[index]
    }
...
function unwrapResultForDisplay(value) {
  if (value && typeof value === 'object' && Object.hasOwn(value, 'ok')) return value.ok
  if (value && typeof value === 'object' && Object.hasOwn(value, 'err')) throw new Error(String(value.err))
  return value
}
...
function validateConfig(config) {
  ...
  for (const [name, tool] of Object.entries(config.tools)) {
    ...
    assert(typeof tool.description === 'string', `view-chat config.tools.${name}.description must be a string`)
    assert(tool.runtime === 'invoke' || tool.runtime === 'call', `view-chat config.tools.${name}.runtime must be "invoke" or "call"`)
    assert(typeof tool.target === 'string' && tool.target.length > 0, `view-chat config.tools.${name}.target must be a non-empty string`)
    assert(Array.isArray(tool.args), `view-chat config.tools.${name}.args must be an array`)
    for (const arg of tool.args) assert(typeof arg === 'string', `view-chat config.tools.${name}.args entries must be strings`)
  }
}
...
export class ViewChat extends HTMLElement {
  ...
  connectedCallback() {
    ...
    assert(this.transcriptElement instanceof HTMLPreElement, 'view-chat missing transcript element')
    assert(this.formElement instanceof HTMLFormElement, 'view-chat missing composer form')
    assert(this.inputElement instanceof HTMLElement, 'view-chat missing message input')
    assert(this.sendButton instanceof HTMLButtonElement, 'view-chat missing send button')
    assert(this.statusElement instanceof HTMLOutputElement, 'view-chat missing status element')
  ...
  async invokeTool(command) {
    ...
    if (!tool) {
      const names = Object.keys(this.tools)
      const suffix = names.length === 0 ? 'No tools are configured.' : `Available tools: ${names.join(', ')}`
      throw new Error(`Unknown tool '${command.name}'. ${suffix}`)
    }

    const args = buildRuntimeArgs(tool.args, command.argv)
    this.appendMessage('tool-call', `${command.name} -> ${tool.runtime}:${tool.target}(${args.map(formatValue).join(', ')})`)

    const raw = tool.runtime === 'invoke'
  ...
  appendMessage(kind, text) {
    ...
    assert(typeof prefix === 'string', `Unknown view-chat message kind '${kind}'`)
    this.messages.push({ kind, text })
    this.renderTranscript()
  }

  renderTranscript() {
    this.transcriptElement.textContent = this.messages.map((message) => {
      const prefixes = {
        system: 'System',
        user: 'You',
        'tool-call': 'Tool call',
