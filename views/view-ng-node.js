...
function kindFromConfigValue(value, fallback = NG.NODE_CODE) {
  if (typeof value === "string")
    return kindFromFormValue(value.trim().toLowerCase(), fallback)
...
function nodeSupportsOutputs(kind) {
  return (
    kind === NG.NODE_CODE || kind === NG.NODE_VALUE || kind === NG.NODE_CALL
  )
}
...
function normalizeTemplatePayload(payload, fallbackKind = NG.NODE_CODE) {
  ...
  const kind =
    rawKind === NG.NODE_VALUE ||
    rawKind === NG.NODE_GOAL ||
    rawKind === NG.NODE_CODE ||
    rawKind === NG.NODE_CALL
      ? rawKind
      : fallbackKind
  ...
  normalized.codeReadOnly =
    kind === NG.NODE_CODE && (!normalized.codePath || Boolean(normalized.code))
  ...
  normalized.outputs = outputList.map((output, index) => {
    ...
    return {
      outputId:
        Number.isFinite(outputId) && outputId > 0 ? outputId : index + 1,
...
export class ViewNgNode extends HTMLElement {
  ...
  _createInitialDraft() {
    if (this.mode === "edit") {
      ...
      draft.inputs = (
        Array.isArray(this.popupProps?.inputLabels)
          ? this.popupProps.inputLabels
          : []
      ).map((name, index) => ({
        inputId: index + 1,
        name: String(name || ""),
      ...
      draft.outputs = (
        Array.isArray(this.popupProps?.outputLabels)
          ? this.popupProps.outputLabels
          : []
      ).map((value, index) => ({
        outputId: index + 1,
      ...
      draft.codeReadOnly = Boolean(
        this.popupProps?.codeReadOnly ??
          (kind === NG.NODE_CODE && !draft.codePath),
      )
  ...
  ensureDraftShape() {
    ...
    if (!nodeSupportsInputs(this.draft.kind)) {
      ...
      !Array.isArray(this.draft.inputs) ||
      this.draft.inputs.length === 0
    ) {
      this.draft.inputs = []
    }
    ...
    if (!nodeSupportsOutputs(this.draft.kind)) {
      ...
      !Array.isArray(this.draft.outputs) ||
      this.draft.outputs.length === 0
    ) {
      this.draft.outputs =
    ...
    if (this.draft.kind === NG.NODE_CODE && !this.draft.codePath) {
      ...
      this.draft.codeStatus =
        this.draft.codeStatus || "Choose a code file to edit."
    }
  }
  ...
  renderForm() {
    ...
    assert(
      ...
      "view-ng-node missing status output",
    )
      ...
      kindSelect.onchange = async () => {
        ...
        if (selectedValue.startsWith("template:")) {
          ...
          const templateEntry = this.templates.find(
            (entry) => String(entry.name || "") === templateName,
          )
          if (!templateEntry) {
            this.setStatus(
    ...
    if (
      ...
      addInput instanceof HTMLButtonElement
    ) {
    ...
    if (
      newOutput instanceof HTMLInputElement &&
      addOutput instanceof HTMLButtonElement &&
      !isValueNode
    ) {
  ...
  applyNodeTemplateToDraft(templateEntry) {
    const payload = normalizeTemplatePayload(
      templateEntry?.data,
      Number(templateEntry?.kind || NG.NODE_CODE),
    )
  ...
  async readGraphEntryById(graphId) {
    ...
    return (
      this.graphEntries.find((entry) => Number(entry.id) === normalized) || null
    )
  }
  ...
  async applyImportNodeGraphRef(graphRef) {
    ...
    if (
      !String(this.draft.name || "").trim() ||
      String(this.draft.name || "").trim() === previousGraphName
    ) {
  ...
  async handleSubmit(event) {
    ...
    const intent = formData.has("remove-input-id")
      ...
      : formData.has("remove-output-id")
        ...
        : String(
            formData.get("intent") ||
              (this.mode === "edit" ? "save-node" : "create-node"),
          )
    ...
    if (intent === "add-input") {
      ...
      if (inputName) {
        const nextId =
          this.draft.inputs.reduce(
            (max, port) => Math.max(max, Number(port.inputId || 0)),
            0,
          ) + 1
    ...
    if (intent.startsWith("remove-input:")) {
      ...
      this.draft.inputs = this.draft.inputs.filter(
        (port) => Number(port.inputId) !== inputId,
      )
      this.renderForm()
      return
    }
    ...
    if (intent === "add-output") {
      ...
      if (this.draft.kind === NG.NODE_VALUE || outputName) {
        const nextId =
          this.draft.outputs.reduce(
            (max, port) => Math.max(max, Number(port.outputId || 0)),
            0,
          ) + 1
    ...
    if (intent.startsWith("remove-output:")) {
      ...
      this.draft.outputs = this.draft.outputs.filter(
        (port) => Number(port.outputId) !== outputId,
      )
      this.renderForm()
      return
    }
    ...
    if (this.draft.kind === NG.NODE_CALL) {
      const graphId = Number(
        formData.get("graph-name") || this.draft.graphId || 0,
      )
