...
function validateNodeGraphRenderConfig(config) {
  requireKeys(config, "view-ng config.config", [
    "theme",
    "layout",
    "node",
    "ports",
  ...
  requireKeys(config.theme, "view-ng config.config.theme", [
    "clear",
    "text",
    "textMuted",
    "selection",
  ...
  for (const key of Object.keys(config.theme))
    requireConfigNumberArray(
      config.theme[key],
      `view-ng config.config.theme.${key}`,
      4,
    )

  requireKeys(config.layout, "view-ng config.config.layout", [
    "gridColumns",
    "gridOriginX",
    "gridOriginY",
    "gridStepX",
  ...
  for (const key of Object.keys(config.layout))
    requireConfigNumber(
      config.layout[key],
      `view-ng config.config.layout.${key}`,
    )

  requireKeys(config.node, "view-ng config.config.node", [
    "width",
    "minHeight",
    "height",
  ])
  for (const key of Object.keys(config.node))
    requireConfigNumber(config.node[key], `view-ng config.config.node.${key}`)

  requireKeys(config.ports, "view-ng config.config.ports", [
    "emptyIconUrl",
    "fullIconUrl",
    "iconSizePx",
    "spacingY",
  ...
  requireConfigString(
    config.ports.emptyIconUrl,
    "view-ng config.config.ports.emptyIconUrl",
  )
  requireConfigString(
    config.ports.fullIconUrl,
    "view-ng config.config.ports.fullIconUrl",
  )
  for (const key of [
    "iconSizePx",
    "spacingY",
    ...
    requireConfigNumber(config.ports[key], `view-ng config.config.ports.${key}`)

  requireKeys(config.edge, "view-ng config.config.edge", [
    "handleMin",
    "handleMax",
    "halfWidthPx",
    "glowPx",
  ...
  for (const key of Object.keys(config.edge))
    requireConfigNumber(config.edge[key], `view-ng config.config.edge.${key}`)

  requireKeys(config.text, "view-ng config.config.text", [
    "fontPx",
    "aa",
    "effect",
    "stroke",
  ...
  for (const key of ["fontPx", "aa", "stroke", "glow", "shadowX", "shadowY"])
    requireConfigNumber(config.text[key], `view-ng config.config.text.${key}`)
  requireKeys(config.text.source, "view-ng config.config.text.source", [
    "metaUrl",
    "atlasUrl",
    "channels",
  ])
  requireConfigString(
    ...
    "view-ng config.config.text.source.metaUrl",
  )
  requireConfigString(
    config.text.source.atlasUrl,
    "view-ng config.config.text.source.atlasUrl",
  )
  requireConfigNumber(
    config.text.source.channels,
    "view-ng config.config.text.source.channels",
  )

  requireKeys(config.nineSlices, "view-ng config.config.nineSlices", [
    "idle",
    "success",
    "error",
    "processing",
  ...
  for (const key of Object.keys(config.nineSlices))
    validateNineSlice(
      ...
      `view-ng config.config.nineSlices.${key}`,
    )
...
export class ViewNg extends HTMLElement {
  ...
  constructor() {
    ...
    this.graphName =
      String(this.getAttribute("graph-name") || "default").trim() || "default"
  ...
  connectedCallback() {
    ...
    if (!this._ready) {
      ...
      assert(
        this.handleElement instanceof HTMLOutputElement,
        "view-ng missing handle output",
      )
      assert(
        this.backendElement instanceof HTMLOutputElement,
        "view-ng missing backend output",
      )
      assert(
        this.statusElement instanceof HTMLOutputElement,
        "view-ng missing status output",
      )
  ...
  _applyViewConfig() {
    ...
    if (!isPlainObject(config))
      throw new Error("view-ng config must be an object")
    if (!isPlainObject(config.config))
      throw new Error("view-ng config.config must be an object")
  ...
  _handleRunProgress(method, payload) {
    ...
    if (nodeId <= 0)
      throw new Error(`view-ng progress ${method} missing nodeId`)
    if (method === "nodeStart" || method === "goalStart")
      this._setExecutionState(nodeId, EXEC_RUNNING, "incoming")
    if (method === "nodeDone" || method === "goalDone")
      this._setExecutionState(nodeId, EXEC_DONE, "connected")
    if (method === "nodeError")
      this._setExecutionState(nodeId, EXEC_ERROR, "connected")
    this.render()
    return okResult()
  }

  _setExecutionState(nodeId, state, edgeMode) {
    const node = this.lastGraph.nodes.find(
      (entry) => Number(entry.id) === Number(nodeId),
    )
    if (!node)
      throw new Error(`view-ng progress references missing node ${nodeId}`)
    ...
    for (const edge of this.lastGraph.edges) {
      ...
      if (edgeMode === "connected" && (incoming || outgoing))
        edge.execState = state
    }
  }
  ...
  _mountHeaderControls() {
    ...
    this._headerControlsElement
      .querySelector('[data-action="save-as"]')
      ?.addEventListener("click", () => {
        void this.saveAs()
      })
    this._headerControlsElement
  ...
  _setInteractionStatusFromState() {
    if (this.connectionDrag) {
      ...
      if (target) {
        this._setStatus(
          `connect ${fixed.nodeId}.${fixed.portId} -> ${target.nodeId}.${target.portId}`,
          "info",
        )
        return
      }
      this._setStatus(
        `drag connection from ${fixed.nodeId}.${fixed.portId}`,
        "info",
      )
      return
    }
    if (this._marqueeRect) {
      this._setStatus(
        `selecting ${this.selectedNodeIds.size} node${this.selectedNodeIds.size === 1 ? "" : "s"}`,
        "info",
      )
      return
    }
    if (this.hoverPick?.kind === "port") {
      this._setStatus(
        `${this.hoverPick.direction} port ${this.hoverPick.nodeId}.${this.hoverPick.portId}`,
        "info",
      )
      return
    }
    if (this.hoverPick?.kind === "edge") {
      ...
      this._setStatus(
        `edge ${edge.from}.${edge.fromOutputId} -> ${edge.to}.${edge.toInputId}`,
        "info",
      )
      return
    }
    ...
    if (this.selectedNodeIds.size > 0) {
      const activeSuffix =
        this.activeNodeId && this.selectedNodeIds.has(this.activeNodeId)
          ? ` (active #${this.activeNodeId})`
          : ""
      this._setStatus(
        `${this.selectedNodeIds.size} node${this.selectedNodeIds.size === 1 ? "" : "s"} selected${activeSuffix}`,
        "info",
      )
      return
    }
  ...
  _hitTestNode(worldX, worldY) {
    for (let i = this.lastGraph.nodes.length - 1; i >= 0; i -= 1) {
      ...
      if (
        worldX < pos.x ||
        worldX > pos.x + size.width ||
        worldY < pos.y ||
        worldY > pos.y + size.height
      )
        continue
      return node
    }
  ...
  _applyMarqueeSelection(additive = false) {
    ...
    if (this.activeNodeId && !this.selectedNodeIds.has(this.activeNodeId)) {
      this.activeNodeId = this.selectedNodeIds.size
  ...
  _pickAtWorld(worldX, worldY) {
    ...
    const portHit = this._hitTestPort(
      worldX,
      worldY,
      this.lastGraph.nodes,
      this.nodeLayout,
    )
    ...
    const edgeHit = this._hitTestEdge(
      worldX,
      worldY,
      this.lastGraph.nodes,
      this.lastGraph.edges,
      this.nodeLayout,
    )
  ...
  _worldDistanceToBezier(x, y, p0, p1, p2, p3) {
    ...
    for (let i = 1; i <= samples; i += 1) {
      ...
      const h =
        ab2 > 1e-6 ? Math.max(0, Math.min(1, (apx * abx + apy * aby) / ab2)) : 0
  ...
  _hitTestPort(worldX, worldY, nodes, posById) {
    ...
    const outputUsage = new Set(
      (this.lastGraph?.edges || []).map(
        (edge) => `${edge.from}:${edge.fromOutputId}`,
      ),
    )
  ...
  _beginConnectionFromPort(portPick) {
    ...
    if (!node || !pos) return
    const p = this._getPortCenter(
      node,
      pos,
      portPick.direction === "input",
      portPick.index,
    )
    this.connectionDrag = {
    ...
    if (portPick.direction === "input" && portPick.connected) {
      const existingEdge = (this.lastGraph?.edges || []).find(
        (edge) =>
          edge.to === portPick.nodeId && edge.toInputId === portPick.portId,
      )
      if (existingEdge) {
        const fromNode = this._getNodeById(existingEdge.from)
        const fromPos = this.nodeLayout.get(existingEdge.from)
        const fromIndex = this._getOutputIndex(
          fromNode,
          existingEdge.fromOutputId,
        )
        const fromPort = this._getPortCenter(
          fromNode,
          fromPos,
          false,
          fromIndex,
  ...
  _updateConnectionHoverTarget(worldX, worldY) {
    ...
    this.connectionDrag.validTarget = this._validateConnectionTarget(
      this.connectionDrag,
      hit,
    )
      ? hit
  ...
  async _applyInputConnect(toNodeId, toInputId, fromNodeId, fromOutputId) {
    ...
    assert(
      this.graphNodes.some((item) => item.id === Number(fromNodeId)),
      `view-ng missing source node ${fromNodeId}`,
    )
  ...
  async _finishConnectionDrag() {
    ...
    const isReconnect =
      drag.mode === "reconnect-input" || drag.mode === "reconnect-output"
    if (target) {
      const from =
        drag.fixed.direction === "output"
          ? drag.fixed
          : {
  ...
  _nextAvailableNodeId() {
    let maxId = 0
    for (const node of this.lastGraph.nodes)
      maxId = Math.max(maxId, Number(node.id || 0))
    return maxId + 1
  }

  _getNodeById(nodeId) {
    return (
      this.lastGraph.nodes.find((node) => Number(node.id) === Number(nodeId)) ||
      null
    )
  }
  ...
  _syncGraphSnapshotFromState({ preserveLayout = true, fit = false } = {}) {
    this.setGraphSnapshot(graphToRenderSnapshot(this.graphNodes), {
      preserveLayout,
      fit,
    })
  }
  ...
  _nodeFromDraft(nodeId, draft, existing = null) {
    ...
    return {
      ...
      codePath:
        kind === NG.NODE_CODE ? String(draft.codePath || "").trim() : "",
      graphId: kind === NG.NODE_CALL ? Number(draft.graphId || 0) : 0,
      graphName:
        kind === NG.NODE_CALL ? String(draft.graphName || "").trim() : "",
      inputs: draftInputs.map((port, index) => ({
        id: Number(port.inputId || port.id || index + 1),
        name: String(port.name || "").trim(),
        srcNodeId: Number(
          existing?.inputs?.find?.(
            (input) =>
              Number(input.id) === Number(port.inputId || port.id || index + 1),
          )?.srcNodeId || 0,
        ),
        srcOutputId: Number(
          existing?.inputs?.find?.(
      ...
      outputs: draftOutputs.map((port, index) => {
        ...
        const existingOutput = existing?.outputs?.find?.(
          (output) => Number(output.id) === outputId,
        )
        return {
          id: outputId,
          name:
            kind === NG.NODE_VALUE
              ? ""
              : String(port.name || existingOutput?.name || "").trim(),
          value:
            port.value === null || existingOutput?.value === null
              ? null
              : String(port.value ?? existingOutput?.value ?? "") || null,
        }
      }),
    }
  }
  ...
  async run() {
    ...
    const compilerRead = unwrap(
      await runtime.invoke("fs/fs::read-text", "ng/compile-graph.lua"),
    )

    const graph = this.getGraph()
    const hasExplicitGoal = graph.some(
      (node) => Number(node.kind) === NG.NODE_GOAL,
    )
...
function __ng_progress(method, nodeId, message)
  host.call(__ng_progress_plugin .. "." .. method, {
    ...
    let generatedSource = ""
    try {
      generatedSource = unwrap(
        await runtime.invoke("lua/lua::run", compilerSource),
      )

      this._setStatus("running generated graph code...", "info")

      const resultText = unwrap(
        await runtime.invoke("lua/lua::run", generatedSource),
      )
  ...
  async new() {
    ...
    await runtime.call("ui.toast.success", {
      message: `Created graph ${payload.path}`,
    })
  }
  ...
  async reload() {
    ...
    await runtime.call("ui.toast.success", {
      message: `Reloaded graph from ${path}`,
    })
  }
  ...
  autoArrangeNodes() {
    ...
    const indegree = new Map(
      nodes.map((node) => [
        Number(node.id),
        incoming.get(Number(node.id)).length,
      ]),
    )
    const queue = nodes
    ...
    for (let i = topo.length - 1; i >= 0; i -= 1) {
      const id = topo[i]
      const successorLayers = outgoing
        .get(id)
    ...
    const sortLayerByNeighborOrder = (layerIds, neighborMap, neighborOrder) => {
      layerIds.sort((a, b) => {
        const aNeighbors = neighborMap
          .get(a)
          .filter((id) => neighborOrder.has(id))
        const bNeighbors = neighborMap
          .get(b)
          .filter((id) => neighborOrder.has(id))
        const aScore = aNeighbors.length
          ? aNeighbors.reduce((sum, id) => sum + neighborOrder.get(id), 0) /
            aNeighbors.length
          : Number.MAX_SAFE_INTEGER
        const bScore = bNeighbors.length
          ? bNeighbors.reduce((sum, id) => sum + neighborOrder.get(id), 0) /
            bNeighbors.length
          : Number.MAX_SAFE_INTEGER
    ...
    const layerWidths = layers.map((layerIds) =>
      Math.max(
        ...layerIds.map((id) => this._measureNodeSize(nodeById.get(id)).width),
        0,
      ),
    )
      ...
      ...layers.map(
        (layerIds) =>
          layerIds.reduce(
            (sum, id) =>
              sum +
              this._measureNodeSize(nodeById.get(id)).height +
              verticalGap,
            0,
          ) - verticalGap,
      ),
    ...
    for (let layerIndex = 0; layerIndex < layers.length; layerIndex += 1) {
      ...
      const totalHeight =
        layerIds.reduce(
          (sum, id) =>
            sum + this._measureNodeSize(nodeById.get(id)).height + verticalGap,
          0,
        ) - verticalGap
    ...
    this._setStatus(
      `auto-arranged ${nodes.length} node${nodes.length === 1 ? "" : "s"} by connections`,
      "success",
    )
  }
  ...
  async deleteSelectedNodes() {
    ...
    this.graphNodes = this.graphNodes
      ...
      .map((node) => ({
        ...
        inputs: node.inputs.map((input) =>
          selected.has(Number(input.srcNodeId))
            ? { ...input, srcNodeId: 0, srcOutputId: 0 }
            : input,
        ),
      }))
    ...
    this._setStatus(
      `deleted ${selected.size} selected node${selected.size === 1 ? "" : "s"}`,
      "success",
    )
  }
  ...
  async saveAs() {
    ...
    await runtime.call("ui.toast.success", {
      message: `Saved graph to ${payload.path}`,
    })
  }
  ...
  setGraphPath(path) {
    assert(
      typeof path === "string" && path.length > 0,
      "view-ng graph path is required",
    )
    this.graphPath = path
    this.graphName = String(path.split("/").pop() || this.graphName)
      .replace(/\.ng\.json$/i, "")
      .replace(/\.json$/i, "")
  ...
  async loadGraphFS(path, notify = true) {
    const readResult = unwrap(
      await runtime.invoke("fs/fs::read-text", path),
      path,
    )
    ...
    if (notify) {
      await runtime.call("ui.toast.success", {
        message: `Loaded graph from ${path}`,
      })
    }
  }
  ...
  async showEditNodePopup() {
    ...
    const payload = unwrap(
      await runtime.call("ui.popup.open", {
        ...
        props: {
          ...
          valueText:
            rawNode.kind === NG.NODE_VALUE
              ? rawNode.outputs[0]?.value || ""
              : "",
          inputLabels: rawNode.inputs.map(
            (input, index) => input.name || `input ${index + 1}`,
          ),
          outputLabels: rawNode.outputs.map((output, index) =>
            rawNode.kind === NG.NODE_VALUE
              ? output.value
    ...
    const next = this._nodeFromDraft(nodeId, payload.draft || {}, {
      ...rawNode,
      x: layout.x,
      y: layout.y,
    })
    this.graphNodes = this.graphNodes.map((item) =>
      item.id === Number(nodeId) ? next : item,
    )
    this._applyGraphMetadataFromNodes(this.graphNodes)
    this._syncGraphSnapshotFromState({ preserveLayout: true, fit: false })
  ...
  _onPointerDown(event) {
    ...
    const hitNode =
      pick?.kind === "node" ? this._getNodeById(pick.nodeId) : null
    this.hoverPick = pick
    ...
    if (event.button === 1 || event.ctrlKey || event.metaKey) {
      ...
      for (const nodeId of this.selectedNodeIds) {
        ...
        if (!pos) continue
        this._dragNodeStartLayout.set(nodeId, {
          x: pos.x,
          y: pos.y,
          width: pos.width,
          height: pos.height,
  ...
  _onPointerMove(event) {
    ...
    this._pointerMoved =
      this._pointerMoved || Math.abs(dxClient) >= 3 || Math.abs(dyClient) >= 3
  ...
  _applyGraphMetadataFromNodes(nodes) {
    ...
    for (const node of nodes) {
      ...
      if (node.kind === NG.NODE_VALUE) {
        this.valueByNode.set(
          nodeId,
          new Map(
            node.outputs.map((output) => [Number(output.id), output.value]),
          ),
        )
      }
      if (node.kind === NG.NODE_CODE)
        this.nodeCodePaths.set(nodeId, node.codePath)
  ...
  loadGraph(graph) {
    ...
    if (this._ready)
      this.setGraphSnapshot(snapshot, { preserveLayout: true, fit: true })
    else this.lastGraph = snapshot
    if (this.handleElement instanceof HTMLOutputElement)
      this.handleElement.textContent = `nodes: ${this.graphNodes.length}`
    if (this.backendElement instanceof HTMLOutputElement)
      this._setBackendStatus("state: frontend", "success")
  }
  ...
  refreshGraphSource() {
    ...
    this._setBackendStatus("state: frontend", "success")
    this._setStatus(
      `rendering graph '${this.graphName}' from frontend state`,
      "info",
    )
  }
  ...
  setGraphSnapshot(graph, { preserveLayout = false, fit = true } = {}) {
    assert(
      ...
      "view-ng requires graph snapshot with nodes and edges arrays",
    )
    ...
    if (preserveLayout) {
      ...
      for (const node of graph.nodes) {
        ...
        if (this.nodeLayout.has(node.id)) {
          this.nodeLayout.set(node.id, {
            ...this.nodeLayout.get(node.id),
            width: size.width,
            height: size.height,
          })
          continue
        }
        const rawNode = this.graphNodes.find(
          (item) => item.id === Number(node.id),
        )
        const layout = this.assets.layout
        this.nodeLayout.set(node.id, {
          x: rawNode
            ? rawNode.x
            : layout.gridOriginX +
              (fallbackIndex % layout.gridColumns) * layout.gridStepX,
          y: rawNode
            ? rawNode.y
            : layout.gridOriginY +
              Math.floor(fallbackIndex / layout.gridColumns) * layout.gridStepY,
          width: size.width,
          height: size.height,
        })
        fallbackIndex += 1
    ...
    const validNodeIds = new Set(graph.nodes.map((node) => Number(node.id)))
    this.selectedNodeIds = new Set(
      [...this.selectedNodeIds].filter((id) => validNodeIds.has(Number(id))),
    )
    if (!validNodeIds.has(Number(this.activeNodeId)))
      this.activeNodeId = this.selectedNodeIds.size
        ? [...this.selectedNodeIds][this.selectedNodeIds.size - 1]
        : 0
    this.contentBounds = this.calculateContentBounds()
  ...
  _measureTextWidth(text, scale = 1) {
    ...
    const atlasSize = Math.max(
      1,
      this.textAtlas?.atlasSize || this.assets.text.fontPx,
    )
  ...
  _getStoredNodeValue(nodeId, outputId) {
    return String(
      this.valueByNode.get(Number(nodeId))?.get(Number(outputId)) || "",
    )
  }
  ...
  _measureNodeSize(node) {
    ...
    const titleWidth = this._measureTextWidth(
      this._getNodeTitleLabel(node),
      titleScale,
    )
    const stateWidth = this._measureTextWidth(
      this._getNodeStateLabel(node),
      portScale,
    )
    ...
    for (let i = 0; i < (node.inputCount || 0); i++) {
      ...
      leftLabelWidth = Math.max(
        leftLabelWidth,
        this._measureTextWidth(
          this._getPortLabel(node.id, "input", inputId, i),
          portScale,
        ),
      )
    }
    ...
    for (let i = 0; i < (node.outputCount || 0); i++) {
      ...
      rightLabelWidth = Math.max(
        rightLabelWidth,
        this._measureTextWidth(label, portScale),
      )
    }
    const bodyWidth =
      inputInsetX +
    ...
    const width = Math.max(
      minWidth,
      Math.ceil(Math.max(headerWidth, bodyWidth)),
    )
  ...
  _getPortCenter(node, pos, isInput, portIndex) {
    ...
    return {
      x: isInput
        ? pos.x + cfg.inputInsetX
        : pos.x + nodeSize.width - cfg.outputInsetX,
  ...
  _getPortLabel(nodeId, direction, portId, index) {
    ...
    const dict = labels
      ? labels[direction === "input" ? "inputs" : "outputs"]
      : null
    if (dict && dict[portId] !== undefined) return String(dict[portId])
    if (dict && dict[String(portId)] !== undefined)
      return String(dict[String(portId)])
  ...
  _getInputIndex(node, inputId) {
    return Math.max(
      0,
      node.inputs.findIndex((port) => port.inputId === inputId),
    )
  }
  ...
  _getOutputIndex(node, outputId) {
    return Math.max(
      0,
      node.outputs.findIndex((port) => port.outputId === outputId),
    )
  }
  ...
  _drawEdges(nodes, edges, posById, width, height, view) {
    ...
    for (const edge of edges) {
      ...
      const p0 = this._getPortCenter(
        fromNode,
        from,
        false,
        this._getOutputIndex(fromNode, edge.fromOutputId),
      )
      const p3 = this._getPortCenter(
        toNode,
        to,
        true,
        this._getInputIndex(toNode, edge.toInputId),
      )
      const h = Math.max(
        edgeCfg.handleMin,
        Math.min(edgeCfg.handleMax, Math.abs(p3.x - p0.x) * 0.5),
  ...
  _getNodeSkinKey(node) {
    ...
    const hovered =
      this.hoverPick?.kind === "node" &&
      Number(this.hoverPick.nodeId) === Number(node.id)
  ...
  _drawNodes(nodes, posById, width, height, view) {
    ...
    for (const node of nodes) {
      ...
      batches
        .get(key)
        .push({ x: pos.x, y: pos.y, width: size.width, height: size.height })
    }
    const drawOrder = [
      "idle",
      "success",
  ...
  _drawPickOverlay(pick, nodes, edges, posById, width, height, view) {
    ...
    if (pick.kind === "port") {
      ...
      const color = this.connectionDrag
        ? this.connectionDrag.validTarget &&
          this.connectionDrag.validTarget.nodeId === pick.nodeId &&
          this.connectionDrag.validTarget.portId === pick.portId &&
          this.connectionDrag.validTarget.direction === pick.direction
          ? validColor
          : invalidColor
        : activeColor
    ...
    if (pick.kind === "edge" && pick.edge) {
      ...
      const p0 = this._getPortCenter(
        ...
        fromPos,
        false,
        this._getOutputIndex(fromNode, pick.edge.fromOutputId),
      )
      const p3 = this._getPortCenter(
        toNode,
        toPos,
        true,
        this._getInputIndex(toNode, pick.edge.toInputId),
      )
      const h = Math.max(
        edgeCfg.handleMin,
        Math.min(edgeCfg.handleMax, Math.abs(p3.x - p0.x) * 0.5),
      ...
      [
        {
          ...
          color: [selection[0], selection[1], selection[2], 0.14],
        },
      ],
      width,
      height,
  ...
  _drawRectFill(rects, width, height, view) {
    ...
    gl.vertexAttribDivisor(2, 1)
    gl.uniformMatrix3fv(
      gl.getUniformLocation(this.rectFillProgram, "u_view"),
      false,
      view,
    )
    gl.uniform2f(
      gl.getUniformLocation(this.rectFillProgram, "u_viewportPx"),
      width,
      height,
    )
    gl.drawArraysInstanced(gl.TRIANGLE_STRIP, 0, 4, rects.length)
  ...
  _drawRectOutline(rects, width, height, view) {
    ...
    gl.vertexAttribDivisor(3, 1)
    gl.uniformMatrix3fv(
      gl.getUniformLocation(this.rectProgram, "u_view"),
      false,
      view,
    )
    gl.uniform2f(
      gl.getUniformLocation(this.rectProgram, "u_viewportPx"),
      width,
      height,
    )
    gl.drawArraysInstanced(gl.TRIANGLE_STRIP, 0, 4, rects.length)
  ...
  _drawPortBatch(textureInfo, instances, width, height, view) {
    ...
    gl.uniformMatrix3fv(
      gl.getUniformLocation(this.spriteProgram, "u_view"),
      false,
      view,
    )
    gl.uniform2f(
      gl.getUniformLocation(this.spriteProgram, "u_viewportPx"),
      width,
      height,
    )
  ...
  _drawPorts(nodes, edges, posById, width, height, view) {
    ...
    const half = iconSize * 0.5
    const outputUsage = new Set(
      edges.map((edge) => `${edge.from}:${edge.fromOutputId}`),
    )
    ...
    for (const node of nodes) {
      ...
      for (let i = 0; i < node.inputCount; i++) {
        ...
        pushInstance(
          input?.srcNodeId ? fullInstances : emptyInstances,
          p.x,
          p.y,
        )
      }
      for (let i = 0; i < node.outputCount; i++) {
        ...
        pushInstance(
          outputUsage.has(`${node.id}:${output.outputId}`)
            ? fullInstances
            : emptyInstances,
          p.x,
          p.y,
    ...
    this._drawPortBatch(
      this.portTextures.empty,
      new Float32Array(emptyInstances),
      width,
      height,
      view,
    )
    this._drawPortBatch(
      this.portTextures.full,
      new Float32Array(fullInstances),
      width,
      height,
      view,
    )
  ...
  _drawLabels(nodes, posById, width, height, view) {
    ...
    const aa = Math.min(
      32.0,
      Math.max(6.0, Number(this.assets.text.aa) * this.scale),
    )
    ...
    gl.uniform1f(
      ...
      Number(this.assets.text.stroke),
    )
    gl.uniform1f(
      gl.getUniformLocation(this.textProgram, "uGlow"),
      Number(this.assets.text.glow),
    )
    gl.uniform2f(
      gl.getUniformLocation(this.textProgram, "uShadowPx"),
      Number(this.assets.text.shadowX),
      Number(this.assets.text.shadowY),
    )
    gl.uniform2f(
    ...
    for (const node of nodes) {
      ...
      drawText(
        state,
        Math.max(pos.x + padX + 56, pos.x + size.width - padX - stateWidth),
        y,
        portScale,
      )
      for (let i = 0; i < node.inputCount; i++) {
        ...
        drawText(
          this._getPortLabel(
            node.id,
            "input",
            node.inputs[i]?.inputId ?? i + 1,
            i,
          ),
          p.x + iconHalf + labelOffset,
          p.y + portPx * 0.35,
          portScale,
        )
      }
      for (let i = 0; i < node.outputCount; i++) {
        ...
        drawText(
          label,
          p.x - iconHalf - labelOffset - labelWidth,
          p.y + portPx * 0.35,
          portScale,
        )
      }
...
function requireNumber(value, label) {
  assert(
    ...
    `view-ng graph ${label} must be a finite number`,
  )
  return value
}
...
function requireOutputValue(value, label) {
  assert(
    ...
    `view-ng graph ${label} must be a string or null`,
  )
  return value
}
...
function cloneNode(node) {
  assert(
    ...
    "view-ng graph node must be an object",
  )
  const id = requireNumber(node.id, "node.id")
  const kind = requireNumber(node.kind, `node ${id}.kind`)
  assert(
    NG_KIND.has(kind),
    `view-ng graph node ${id} kind must be one of 1, 2, 3, 4`,
  )
  assert(
    Array.isArray(node.inputs),
    `view-ng graph node ${id}.inputs must be an array`,
  )
  assert(
    Array.isArray(node.outputs),
    `view-ng graph node ${id}.outputs must be an array`,
  )
  return {
    id,
    kind,
    ...
    inputs: node.inputs.map((input, index) =>
      cloneInput(input, `node ${id}.inputs[${index}]`),
    ),
    outputs: node.outputs.map((output, index) =>
      cloneOutput(output, `node ${id}.outputs[${index}]`),
    ),
  }
}
...
export function cloneNgGraph(graph) {
  ...
  for (const node of nodes) {
    ...
    for (const input of node.inputs) {
      assert(
        input.id > 0,
        `view-ng graph node ${node.id} input id must be positive`,
      )
      assert(
        !inputIds.has(input.id),
        `view-ng graph node ${node.id} has duplicate input id ${input.id}`,
      )
      inputIds.add(input.id)
      if (input.srcNodeId !== 0)
        assert(
          ids.has(input.srcNodeId),
          `view-ng graph input ${node.id}.${input.id} references missing source node ${input.srcNodeId}`,
        )
    }
    ...
    for (const output of node.outputs) {
      assert(
        output.id > 0,
        `view-ng graph node ${node.id} output id must be positive`,
      )
      assert(
        !outputIds.has(output.id),
        `view-ng graph node ${node.id} has duplicate output id ${output.id}`,
      )
