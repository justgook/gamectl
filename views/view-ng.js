...
function validateNodeGraphRenderConfig(config) {
    ...
    for (const key of Object.keys(config.theme))
        requireConfigNumberArray(config.theme[key], `view-ng config.config.theme.${key}`, 4)
    ...
    for (const key of Object.keys(config.layout))
        requireConfigNumber(config.layout[key], `view-ng config.config.layout.${key}`)
    ...
    for (const key of Object.keys(config.nineSlices))
        validateNineSlice(config.nineSlices[key], `view-ng config.config.nineSlices.${key}`)
...
export class ViewNg extends HTMLElement {
    ...
    _hitTestNode(worldX, worldY) {
        for (let i = this.lastGraph.nodes.length - 1; i >= 0; i -= 1) {
            ...
            if (worldX < pos.x || worldX > pos.x + size.width || worldY < pos.y || worldY > pos.y + size.height)
                continue
            return node
        }
        return null
        ...
        return {
            ...
            inputs: draftInputs.map((port, index) => ({
                ...
                srcNodeId: Number(
                    existing?.inputs?.find?.(
                        (input) => Number(input.id) === Number(port.inputId || port.id || index + 1),
                    )?.srcNodeId || 0,
                ),
                srcOutputId: Number(
                    existing?.inputs?.find?.(
            ...
            outputs: draftOutputs.map((port, index) => {
                ...
                return {
                    ...
                    value: port.value === null ? null : String(port.value ?? existingOutput?.value ?? "") || null,
                }
            }),
        }
    }
  ...
  host.call(__ng_progress_plugin .. "." .. method, {
    ...
    autoArrangeNodes() {
        ...
        const sortLayerByNeighborOrder = (layerIds, neighborMap, neighborOrder) => {
            layerIds.sort((a, b) => {
                ...
                const aScore = aNeighbors.length
                    ? aNeighbors.reduce((sum, id) => sum + neighborOrder.get(id), 0) / aNeighbors.length
                    : Number.MAX_SAFE_INTEGER
                const bScore = bNeighbors.length
                    ? bNeighbors.reduce((sum, id) => sum + neighborOrder.get(id), 0) / bNeighbors.length
                    : Number.MAX_SAFE_INTEGER
        ...
        const layerWidths = layers.map((layerIds) =>
            Math.max(...layerIds.map((id) => this._measureNodeSize(nodeById.get(id)).width), 0),
        )
        const maxLayerHeight = Math.max(
            ...layers.map(
                (layerIds) =>
                    layerIds.reduce(
                        (sum, id) => sum + this._measureNodeSize(nodeById.get(id)).height + verticalGap,
                        0,
                    ) - verticalGap,
            ),
            0,
        ...
        for (let layerIndex = 0; layerIndex < layers.length; layerIndex += 1) {
            ...
            const totalHeight =
                layerIds.reduce((sum, id) => sum + this._measureNodeSize(nodeById.get(id)).height + verticalGap, 0) -
                verticalGap
    ...
    async deleteSelected() {
        ...
        this.graphNodes = this.graphNodes
            ...
            .map((node) => ({
                ...node,
                inputs: node.inputs.map((input) =>
                    selected.has(Number(input.srcNodeId)) ? { ...input, srcNodeId: 0, srcOutputId: 0 } : input,
                ),
            }))
            ...
            await runtime.call("ui.popup.open", {
                ...
                props: {
                    ...
                    outputLabels: rawNode.outputs.map((output, index) =>
                        rawNode.kind === NG.NODE_VALUE ? output.value : output.name || `output ${index + 1}`,
                    ),
                },
            }),
        )
        ...
        for (const node of nodes) {
            ...
            this.portLabels.set(nodeId, {
                inputs: Object.fromEntries(
                    node.inputs.map((input, index) => [String(input.id), input.name || `input ${index + 1}`]),
                ),
                outputs: Object.fromEntries(
                    node.outputs.map((output, index) => [
                        String(output.id),
                        node.kind === NG.NODE_VALUE ? "" : output.name || `output ${index + 1}`,
                    ]),
                ),
            })
        ...
        if (preserveLayout) {
            ...
            for (const node of graph.nodes) {
                ...
                this.nodeLayout.set(node.id, {
                    x: rawNode
                        ? rawNode.x
                        : layout.gridOriginX + (fallbackIndex % layout.gridColumns) * layout.gridStepX,
                    y: rawNode
                        ? rawNode.y
                        : layout.gridOriginY + Math.floor(fallbackIndex / layout.gridColumns) * layout.gridStepY,
                    width: size.width,
                    height: size.height,
                })
                fallbackIndex += 1
    ...
    _viewMatrix() {
        return new Float32Array([this.scale, 0, 0, 0, this.scale, 0, this.offsetX, this.offsetY, 1])
    }
    ...
    _measureNodeSize(node) {
        ...
        for (let i = 0; i < (node.inputCount || 0); i++) {
            ...
            leftLabelWidth = Math.max(
                leftLabelWidth,
                this._measureTextWidth(this._getPortLabel(node.id, "input", inputId, i), portScale),
            )
        }
    ...
    _drawPickOverlay(pick, nodes, edges, posById, width, height, view) {
        ...
        if (pick.kind === "port") {
            ...
            this._drawPortBatch(
                ...
                new Float32Array([p.x - half, p.y - half, iconSize, iconSize, 0, 0, 1, 1]),
                width,
                height,
                view,
            )
    ...
    _drawPorts(nodes, edges, posById, width, height, view) {
        ...
        for (const node of nodes) {
            ...
            for (let i = 0; i < node.outputCount; i++) {
                ...
                pushInstance(
                    outputUsage.has(`${node.id}:${output.outputId}`) ? fullInstances : emptyInstances,
                    p.x,
                    p.y,
                )
            }
    ...
    _drawLabels(nodes, posById, width, height, view) {
        ...
        for (const node of nodes) {
            ...
            for (let i = 0; i < node.inputCount; i++) {
                const p = this._getPortCenter(node, pos, true, i)
                drawText(
                    this._getPortLabel(node.id, "input", node.inputs[i]?.inputId ?? i + 1, i),
