...
export class ViewImage extends ViewCanvasBase {
  ...
  connectedCallback() {
    ...
    assert(
      ...
      "view-image missing status output",
    )
    assert(
      this.pathElement instanceof HTMLOutputElement,
      "view-image missing path output",
    )
  ...
  setStatus(text, tone = null) {
    assert(
      this.statusElement instanceof HTMLOutputElement,
      "view-image status output is not initialized",
    )
      ...
      const source =
        ext === "qoi"
          ...
          : await createImageBitmap(
              new Blob([bytes], { type: mimeTypeForPath(this.path) }),
            )
