---
title: Player Experience
summary: What players perceive, anticipate, decide, feel, and learn from moment to moment and across the whole game.
eyebrow: Design
status: in-progress
---

## Experience thesis

> **In progress** — **Familiarity without repetition.** The game should awaken the player's memory of 16-bit-era action games without asking them to recognize a specific existing character, story, or world. The player feels at home, but cannot predict the actual journey.

## Recognition before explanation

The cast should use strong, original archetypes and silhouettes. A player who has never seen these characters should still form an immediate expectation of each character's fantasy and possible play style.

The world should produce the same anticipatory recognition. A wasteland, forest, or factory can imply a larger action-game journey before exposition explains why the player is there.

## Character-choice anticipation

Choosing a character is the first meaningful promise: different characters should create materially different play experiences, not merely cosmetic variation or small statistical bonuses.

The choice must not accidentally punish an uninformed player. It remains unresolved how much of each character's play style is explained before selection and how easily the player can reconsider the choice.

## Opening emotional arc

1. **Recognition:** “I understand the kind of world and heroes I am looking at.”
2. **Anticipation:** “My choice will change what happens and how I play.”
3. **Dislocation:** “This feels like a remembered game, but I have not seen this journey before.”
4. **Contradiction:** Recovered evidence conflicts with a briefing, memory, assigned role, or accepted order.
5. **Agency:** Questioning identity and purpose changes from a forbidden mistake into a necessary act.

## Narrative agency

> **Accepted** — The player knows from the opening that the crew's autobiographical identity is missing or unreliable. The mystery is not whether something is wrong, but what happened, whether an original person exists, which memories are real, and what the missions serve.

> **Accepted** — The crew begins with an “act first, ask later” orientation. Soldiering is its only stable identity. Crew members are physically capable of refusal, but operational conditioning makes questioning orders or selfhood feel improper, unsafe, and unlike themselves.

The player's first form of rebellion is therefore not defeating the controller—it is recognizing that asking “why?” is possible.

> **Accepted** — Early missions create genuine local benefits while concealing their institutional purpose. Questioning an order is morally difficult because abandoning the mission may hurt people who actually need its stated outcome.

## Opening experience

> **Accepted** — A new game gives control almost immediately. Mission objective and other crew voices arrive over radio during safe movement. The player learns action before receiving the first identity contradiction. The controller calmly labels it legacy corruption; one crew voice hesitates while another normalizes it. The Hub becomes the first quiet narrative pause. Arrival remains player-controlled, connects radio voices to physically present crew, and exposes direct selection shortcuts without a forced tour.

## Representative play

> **Needs evidence** — The action loop is described, but one playable encounter must prove its decisions, feedback, and character-specific variation.

## Mastery

> **In progress** — The intended player enjoys fair, learnable difficulty and should improve at reading threats, executing movement and combat, recognizing route possibilities, and exploiting the selected character's strengths.

> **Open question** — How should mastery differ after ten minutes, two hours, and twenty hours?
