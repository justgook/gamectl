...
decode_bullets :: proc(r: ^Reader, out: ^bullets) -> bool {
	{
	count, ok := read_u32_reader(r)
	if !ok { return false }
	out^ = make(bullets, int(count))
...
read_slot_0_bullets :: proc(pkg: Package) -> (bullets, bool) {
	r, ok := slot_reader(pkg, 0)
	if !ok { return nil, false }
	value: bullets
	if !decode_bullets(&r, &value) { return nil, false }
	return value, true
}
