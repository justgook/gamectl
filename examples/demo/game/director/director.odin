...
trigger :: proc(state: ^State, event: Trigger) -> Result {
	...
	if !ok {
		return Result{rule = INVALID_RULE, narrative = INVALID_TEXT, matched = false, effects = state.effects[:]}
	}
	rule := &state.data.rules[rule_index]
	apply_rule(state, rule_index, event)
	return Result{rule = rule.id, narrative = rule.narrative, matched = true, effects = state.effects[:]}
}
