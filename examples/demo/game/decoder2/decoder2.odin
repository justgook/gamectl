...
decode_bullet_patterns :: proc(r: ^Reader, out: ^bullet_patterns) -> bool {
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_bullet_pattern(r, &out^[i]) {return false}
			}
		}
	}
	return true
...
decode_bullet_pattern :: proc(r: ^Reader, out: ^bullet_pattern) -> bool {
	...
	}
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_bullet(r, &out.bullets[i]) {return false}
			}
		}
	}
	{
		...
		for i in 0 ..< int(count) {
			{
				count, ok := read_u32_reader(r)
				if !ok {return false}
				out.actions[i] = make(action, int(count))
				for i in 0 ..< int(count) {
					{
						if !decode_command(r, &out.actions[i][i]) {return false}
					}
				}
			}
		}
	}
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_fire(r, &out.fires[i]) {return false}
			}
		}
	}
	return true
...
decode_ref_with_params :: proc(r: ^Reader, out: ^ref_with_params) -> bool {
	...
	}
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_expr(r, &out.params[i]) {return false}
			}
		}
	}
	return true
...
decode_bullet :: proc(r: ^Reader, out: ^bullet) -> bool {
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_ref(r, &out.action_refs[i]) {return false}
			}
		}
	}
	return true
...
decode_action :: proc(r: ^Reader, out: ^action) -> bool {
	{
		...
		for i in 0 ..< int(count) {
			{
				if !decode_command(r, &out^[i]) {return false}
			}
		}
	}
	return true
